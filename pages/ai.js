...
const DARK = {
  ...
  green:'#00d17a', red:'#ff4060', blue:'#4da6ff', amber:'#ffaa00', purple:'#9f7eff',
  orange:'#ff7a00',
}
const LIGHT = {
  bg:'#fafafa', surface:'#ffffff', card:'#ffffff', border:'#ebebeb',
  text:'#1a1a2e', text2:'#555e6e', muted:'#9aa0ad',
  green:'#00b068', red:'#e8334a', blue:'#0066ff', amber:'#ff8c00', purple:'#7c4dff',
  orange:'#ff6600',
}
      ...
      <div style={{minHeight:'100vh',background:t.bg,fontFamily:'Inter,sans-serif',color:t.text}}>
        ...
        <header style={{background:dark?'rgba(11,14,22,0.95)':'rgba(255,255,255,0.95)',backdropFilter:'blur(16px)',borderBottom:`1px solid ${t.border}`,padding:'0 24px',height:56,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:100}}>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <button onClick={()=>router.push('/dashboard')} style={{background:t.surface,border:`1px solid ${t.border}`,borderRadius:8,color:t.muted,cursor:'pointer',fontSize:12,padding:'5px 12px',fontFamily:'Inter,sans-serif',fontWeight:600}}>← Dashboard</button>
            <span style={{fontSize:16}}>🤖</span>
            <span style={{fontWeight:800,fontSize:16,color:t.text}}>AI Trading Partner</span>
          </div>
          <button onClick={()=>{const nd=!dark;setDark(nd);localStorage.setItem('pz_dark',String(nd))}} style={{background:t.surface,border:`1px solid ${t.border}`,borderRadius:20,padding:'5px 14px',cursor:'pointer',fontSize:13,color:t.text,fontFamily:'Inter,sans-serif'}}>
            {dark?'☀️ Light':'🌙 Dark'}
          </button>
        </header>
        ...
        <main style={{maxWidth:900,margin:'0 auto',padding:'24px 20px 60px'}}>
          ...
          <div style={{background:t.purple+'0d',border:`1px solid ${t.purple}33`,borderRadius:16,padding:22,marginBottom:24}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
              ...
              <button onClick={fetchBriefing} disabled={bLoading} style={{padding:'5px 14px',background:t.purple+'22',border:`1px solid ${t.purple}44`,borderRadius:8,color:t.purple,cursor:'pointer',fontSize:12,fontFamily:'Inter,sans-serif',fontWeight:600}}>
                {bLoading ? '...' : '🔄 Refresh'}
              </button>
            </div>
            {bLoading
              ...
              : <pre style={{color:t.text2,fontSize:13,lineHeight:1.9,whiteSpace:'pre-wrap',fontFamily:'Inter,sans-serif',margin:0}}>{briefing||'Click Refresh to generate briefing'}</pre>
            }
          </div>
          ...
          <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,overflow:'hidden'}}>
            ...
            <div style={{padding:'10px 16px',borderTop:`1px solid ${t.border}`,borderBottom:`1px solid ${t.border}`,display:'flex',gap:6,flexWrap:'wrap',background:t.surface+'55'}}>
              {QUICK.map(q=>(
                <button key={q} onClick={()=>{setInput(q)}} style={{padding:'4px 12px',background:t.surface,border:`1px solid ${t.border}`,borderRadius:20,color:t.text2,cursor:'pointer',fontSize:11,fontFamily:'Inter,sans-serif'}}>
                  {q}
                </button>
              ))}
            </div>
            ...
            <div style={{padding:'12px 16px',display:'flex',gap:8}}>
              <input
                ...
                style={{flex:1,background:t.surface,border:`1px solid ${t.border}`,borderRadius:10,color:t.text,fontSize:13,padding:'10px 14px',fontFamily:'Inter,sans-serif',outline:'none'}}
              />
              <button onClick={sendChat} disabled={!input.trim()||cLoading} style={{padding:'10px 20px',background:!input.trim()||cLoading?t.surface:`linear-gradient(135deg,${t.accentC},${t.purple})`,border:`1px solid ${t.border}`,borderRadius:10,color:!input.trim()||cLoading?t.muted:'#fff',fontWeight:700,cursor:!input.trim()||cLoading?'not-allowed':'pointer',fontSize:13,fontFamily:'Inter,sans-serif'}}>
                Send
              </button>
            </div>
          </div>
