...
function WatchlistTab({t, at}) {
  ...
  async function fetchPrices() {
    try {
      ...
      // Crypto prices via live-prices API instead
      if (cryptoSyms.length) {
        const r = await fetch('/api/live-prices')
        const d = await r.json()
        if (d.crypto) Object.assign(newPrices, d.crypto)
      }
      setPrices(newPrices)
    } catch {}
  }
  ...
  const CRYPTO_SYMS = ['BTC','ETH','SOL','XRP']
...
function AlertsTab({t}) {
  ...
  const SYMS_CRYPTO = ['BTC','ETH','SOL','XRP']
